...
- `src/api/routes/` — REST API routes: baby, entries (CRUD), media (proxy), summary
...
POST   /api/v1/entries              — create text entry
PATCH  /api/v1/entries/:id/text     — update entry text
PATCH  /api/v1/entries/:id/date     — update event date
DELETE /api/v1/entries/:id          — delete entry
GET    /api/v1/media/:fileId        — proxy Telegram file
POST   /api/v1/summary             — generate monthly AI summary
```
...
**Screens**: Feed (entry timeline), Detail (full entry view with photo lightbox), Create (text + date), Edit (text + date), Summary (monthly AI summary).
...
- Dockerfile has 4 stages: `deps` → `builder` (backend tsc) → `miniapp-builder` (Vite build) → `runner`
